...
### GD算法中的术语补充：
- **batch_size** ：在mini-BGD算法中每个epoch里面抽取batch进行训练，其中每个batch的大小是相同的，用batch_size表示
- **前向传播** ( forward propagation / forward pass ) : 按顺序(从输入层到输出层)计算和存储神经网络中每层的结果。在BGD算法中只有最后一层，即cost函数值。
- **反向传播** ( backward propagation，简称 **BP** )指的是计算神经网络参数梯度的方法。其原理是基于微积分中的链式规则，按相反的顺序从输出层到输入层遍历网络，依次计算每个中间变量和参数的梯度，用于参数的更新。
- **iteration** ：一个迭代相当于使用一个batch训练一次，在一个epoch里面iteration的数量就是$\frac{总数}{batch\_size}$,该概念同样适用于神经网络中，每个iteration的结果都会作为下一次iteration的初始值。总的来说，一个迭代 = 一个（batch_size）数据正向通过（forward）+ 一个（batch_size）数据反向（backward）
- **epoch** ： 指的是训练过程中全部样本数据将被“轮”多少次。
- **动量（momentum）**：随机梯度下降(SGD)学习过程有时会很慢。比如在穿越山谷时，使用SGD则会在山谷壁来回的摆动（如下图）。动量算法积累了之前梯度指数级衰减的移动平均，并且继续沿该方向移动。v表示动量，超参数α：α [0,1) 决定了之前梯度的贡献衰减得有多快。 
![](2023-02-25-01-08-07.png) 

<br>
补充资料 from：  

- https://zhuanlan.zhihu.com/p/445365836
- https://zhuanlan.zhihu.com/p/437894581